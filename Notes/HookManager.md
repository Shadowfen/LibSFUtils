...
Unlike the ESO hook APIs, hooks created by `HookManager` remain permanently registered with the game. Enabling or disabling a hook simply determines whether its callback executes.
...
The underlying ESO hook remains registered, but the callback is skipped.
...
- `PreHook()` may cancel the original function by returning `true`.
